...
long long getMaxScore(int left, int right) {
    ...
    int l = mid, r = mid;
    long long maxscore = arr[mid], curheight = arr[mid];
    ...
    // 코드의 길이 줄일 수 있음
        ...
        if (a >= b) {
            l--;
            curheight = min(curheight, a);
        }
        else {
            r++;
            curheight = min(curheight, b);
        }
        maxscore = max(maxscore, curheight * (r-l+1));
    }
...
양쪽의 두 배열에서 최대 넓이를 구함
가운데 원소를 포함한 최대 널이를 구함, 이 때 구간을 높이 큰 쪽으로 한 원소씩 늘려나가며 최대 점수 찾음
세 값 중 가장 큰 것을 반환
*/